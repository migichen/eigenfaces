...
% figure;
% imgsize=size(img);
% for i=1:min(results, 25)
%     subplot(5,5,i);
%     range = result_range{I(i)};
%     subimg = img(uint32(range(2)):uint32(min(imgsize(1), range(2)+range(4)-1)), uint32(range(1)):uint32(min(imgsize(2), range(1)+range(3)-1)));
% %     a=[(range(1)), uint32(min(imgsize(1), range(1)+range(3)-1)), uint32(range(2)),uint32(min(imgsize(2), range(2)+range(4)-1))]
%     imshow(subimg, [0 255]);
%     title(Y(i));
% end
